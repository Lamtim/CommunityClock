...
class AlarmAdapter(val listener:OnAlarmToggleListener) : RecyclerView.Adapter<AlarmAdapter.AlarmViewHolder>() {
    ...
    override fun onBindViewHolder(alarmViewHolder: AlarmAdapter.AlarmViewHolder, i: Int) {
        alarmViewHolder.bind(mAlarms!![i],listener)
    }
    ...
    class AlarmViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private var toggle: SwitchCompat = itemView.findViewById(R.id.sw_toggle)

        fun bind(alarm: Alarm?, listener: OnAlarmToggleListener) {
            if (alarm != null) {
                time.text = alarm.formatTime()
                toggle.setOnCheckedChangeListener { buttonView, isChecked ->
                    alarm.isOn = isChecked
                    listener.onAlarmToggle(alarm)
                }
                toggle.isChecked = alarm.isOn
            }
        }
    }
}
